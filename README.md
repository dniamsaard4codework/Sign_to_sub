...
│
├── ForcedAlignment/                        ← Task 2 scale-up to 1,132 clips (see ForcedAlignment/README.md)
...
│   │
│   │  ── Pre-computed inputs
│   ├── subtitles/04.vtt                    ← CC_Input VTT (119 cues, in repo)
│   ├── subtitles_gloss_cc_time/04.vtt      ← Gloss_Input + CC_Input timestamps (in repo)
│   ├── segmentation_output/E4s-1_30_50/    ← SIGN tier 2780 segments (NOT in repo — regenerated by Step D)
│   │
│   │  ── Embeddings (NOT in repo — regenerated by Step E)
...
│   │
│   │  ── Alignment outputs (cached in repo — used by Evaluation-Only Quick Start)
...
│   │
│   │  ── Ablation outputs (Progress_09052026, cached in repo)
│   ├── ablation/
...
│   │
│   │  ── Comparison/best EAFs (built by add_*_to_eaf.py, cached in repo)
...
├── patches/                                ← Out-of-tree patches (e.g. fairseq_signclip_windows.patch — see Step 5)
...
py -3.11 --version       # Python 3.11.x  (NOT 3.12, NOT 3.13, NOT 3.14)
nvidia-smi               # GPU driver visible (optional — pipeline runs on CPU too, just slowly)
```

> **Python 3.11 specifically required** (not 3.12+) because `mediapipe==0.10.21` has no wheels for newer Python.
>
> - **Don't have Python 3.11?** Download from [python.org/downloads/release/python-3119/](https://www.python.org/downloads/release/python-3119/) — the Windows installer registers `py -3.11`. Alternatively use [uv](https://github.com/astral-sh/uv) (`uv python install 3.11`) or `pyenv-win`.
> - **`py -3.11` shows "No suitable Python runtime found"?** Python 3.11 isn't installed yet — install it first using one of the methods above.
