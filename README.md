...
# CPU-only (works everywhere, recommended for most users)
uv sync

# With CUDA GPU support (Linux only, requires NVIDIA drivers ≥ 525)
uv sync --extra cuda
```
...
| Section | Content |
| --- | --- |
| 1 | Environment & path setup — `sys.path` mechanics, CUDA detection |
...
| 5 | Running DP alignment — full algorithm walkthrough, hyperparameter guide |
| 6 | Writing output EAF and WebVTT — XML manipulation, VTT spec |
| 7 | Evaluation — self-evaluation strategy, F1@IoU metrics, visualisation chart |
| 8 | Timeline visualisation — 3-track comparison chart |
| 9 | Summary and next steps — embedding extension, full pipeline, parameter tuning |
...
> **How to read:** Blue bars (Track 1) are individual sign gestures from the Gloss Labeling tier. Red bars (Track 2) are the original subtitle timestamps. Green bars (Track 3) are the DP-corrected timestamps, snapped to sign segment boundaries.

### Evaluation Metrics

**Panel A** compares F1 scores and frame-level accuracy between the notebook's self-evaluation run and the BOBSL benchmark from the paper.
**Panel B** shows the mean and median timing offsets (GT − predicted) for start and end times.

![Evaluation Metrics](assets/evaluation_metrics.png)

> **Note:** The self-evaluation compares DP-aligned timestamps against the original CC\_Aligned tier (the reference). High F1 scores indicate the DP aligner made only small corrections to the already well-aligned manual annotations.
