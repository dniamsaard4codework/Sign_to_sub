...
> ⚠️ **Patches required after clone:** `fairseq_signclip/` ถูก gitignore ไว้ (29 GB ใหญ่เกินสำหรับ git)
> หลัง `git clone` ที่ Step 5 คุณจะได้ **vanilla upstream** ที่ยังไม่มี Windows path fix —
> ต้องแก้ไฟล์ตามตาราง [Changes from Upstream](#changes-from-upstream) ด้วยตัวเอง
> มิฉะนั้น `align_gloss_labels.py` / `extract_episode_features.py` จะ error
> เกี่ยวกับ path บน Windows ดู §"Changes from Upstream" สำหรับรายชื่อไฟล์
> ที่ต้องแก้
