...
The notebook `example_alignment_usage.ipynb` provides a complete, section-by-section walkthrough of **Stage 3 (DP Alignment)** using the Thai Sign Language example in `data/example_alignment/`. Every code cell is preceded by a detailed markdown explanation covering the algorithm, the data structures, codebase internals, and edge cases.

**Notebook sections:**

| Section | Content |
| --- | --- |
| 1 | Environment & path setup — `sys.path` mechanics, CUDA/GPU detection |
| 2 | Loading the ELAN file — XML schema, tier structure, Thai filename handling |
| 3 | Extracting cues and sign segments — the SEA dict format, tier mismatch explanation |
| 4 | Numba JIT warmup — LLVM compilation, caching, why it's needed |
| 5 | Simulated misalignment + DP alignment — why pre-aligned data gives trivial results; Gaussian noise (σ=1s) simulation; full DP algorithm walkthrough |
| 6 | Writing output EAF and WebVTT — XML manipulation, VTT spec |
| 7 | Evaluation — two-run comparison (noisy baseline vs DP-aligned), F1@IoU metrics, before/after visualisation chart |
| 8 | Timeline visualisation — 3-track comparison chart (sign segments / original / DP-aligned) |
| 9 | Summary and next steps — actual result numbers, embedding extension, full pipeline, parameter tuning |

**Outputs generated by the notebook:**

```text
data/example_alignment/
├── *_updated.eaf           ← ELAN file with SUBTITLE_SHIFTED tier added
...
The three-track chart shows a 60-second window of the video with sign segments, original CC\_Aligned timestamps, and DP-aligned timestamps overlaid:

![Alignment Timeline](assets/alignment_visualization.png)

> **How to read:** Blue bars (Track 1) are individual sign gestures from the Gloss Labeling tier. Red bars (Track 2) are the original CC\_Aligned subtitle timestamps (ground truth). Green bars (Track 3) are the DP-aligned timestamps after correcting simulated Gaussian misalignment (σ=1s). Green bar edges snap to sign-segment boundaries.

### Evaluation Metrics

**Panel A** shows F1 scores and frame-level accuracy for three series: noisy baseline (before DP), DP-aligned output (after DP), and the BOBSL paper benchmark.
...
> **Results (this notebook run):** Starting from Gaussian-noisy input (σ=1s), the DP aligner improves frame accuracy from **79.68% → 91.03%** (+11.4 pp) and F1@0.50 from **85.23% → 94.07%** (+8.8 pp). The median absolute start offset drops to **0.00 s** — most cues snap exactly back to the original sign-segment boundaries.
