...
สำหรับวิดีโอตัวอย่าง 11 นาทีนี้ได้ **~419 sign segments** (ด้วยค่า SIGN_B=30, SIGN_O=70)
...
**ขนาด matrix สำหรับวิดีโอตัวอย่าง (mode `text_embedding`):**

| Matrix | ขนาด | คำอธิบาย |
| --- | --- | --- |
| subtitle text embeddings | **(172 × 384)** | 172 cues, 384 มิติต่อ cue |
| sign segment embeddings | **(419 × 384)** | 419 sign segments, 384 มิติต่อ segment |
| similarity matrix | **(172 × 419)** | dot product ทุกคู่ (i, j) หลัง normalize |
...
**ความหมายของแต่ละ term:**

| Term | ความหมาย | น้ำหนัก |
| --- | --- | --- |
| `\|start(tᵢ) − start(sₗ)\|` | ลงโทษถ้า start ไม่ตรงกัน | 1 (implicit) |
| `\|end(tᵢ) − end(sᵣ)\|` | ลงโทษถ้า end ไม่ตรงกัน | 1 (implicit) |
| `w_dur × \|dur(tᵢ) − dur(sₗ:ᵣ)\|` | ลงโทษถ้าความยาวต่างกัน | **w_dur = 1** |
| `w_gap × Σgaps` | ลงโทษถ้า sign segments ที่จับคู่ด้วยมีช่องว่างระหว่างกันมาก | **w_gap = 5** |
| `− w_sim × ΣM[i,j]` | **ให้รางวัล** ถ้า embedding คล้ายกัน (ลบเพราะ minimize) | **w_sim = 10** |

**Constraint พิเศษ: max_gap**
ถ้า sign segment สองอันมีช่องว่างระหว่างกัน > `max_gap = 10.0 วินาที` → ห้ามจับรวมใน span เดียวกัน ป้องกัน DP จับ subtitle cue หนึ่งกับ sign segments ที่กระจายอยู่คนละส่วนของวิดีโอ
...
├── poses/04.pose                              ← Stage 0 (keypoints ทุก frame)
├── subtitles/04.vtt                           ← Input VTT (CC tier, 172 cues)
├── ground_truth/04.vtt                        ← GT (aligned_output.vtt, 172 cues)
├── segmentation/E4s-1_30_70/04.eaf            ← Stage 1 (SIGN tier, ~419 segments)
├── embeddings/text_embeddings.npy             ← Stage 2 (172 × 384 subtitle embeddings)
├── embeddings/sign_embeddings.npy             ← Stage 2 (419 × 384 sign embeddings)
├── embeddings/sim_matrix.npy                  ← Stage 2 (172 × 419 similarity matrix)
...
**⚠ ทำไม 89.41% ≠ 50.68% ของ Paper:**

ตัวเลข F1@0.50 = 89.41% ของ demo นี้ **ไม่สามารถเปรียบเทียบโดยตรง** กับ BOBSL 50.68% ในบทความ เหตุผลคือ **ประเภทของ Ground Truth ต่างกัน:**

| | Demo นี้ | Paper (BOBSL) |
| --- | --- | --- |
| GT คืออะไร | Reference alignment (สร้างโดยอ้างอิง sign segment เดิม) | Human annotation อิสระ (annotator ไม่รู้ sign segment) |
| ความยาก | **ง่ายกว่า** — GT มีโครงสร้างเดียวกับ input | **ยากกว่า** — GT เป็น independent benchmark |
| เหมาะสำหรับ | ตรวจสอบว่า pipeline ทำงานถูกต้อง | วัดคุณภาพ alignment จริงๆ |
...
### การเปรียบเทียบ Parameter: Demo นี้ vs ค่า Default ของ Paper

| Parameter | Paper (BOBSL) | Demo นี้ (Thai SL) | เหตุผลที่ต่าง |
| --- | --- | --- | --- |
| SIGN_B | 30 | **30** | เหมือนกัน — ใช้ default |
| SIGN_O | 50 | **70** | โมเดล BSL over-segments วิดีโอภาษามือไทย → เพิ่ม threshold เพื่อ merge segment ที่ฟุ้ง |
| w_dur | 1 | **1** | เหมือนกัน |
| w_gap | 5 | **5** | เหมือนกัน |
| w_sim | 10 | **10** | เหมือนกัน |
| window_size | 50 | **50** | เหมือนกัน |
| max_gap | 10s | **10s** | เหมือนกัน |

**สรุป:** ใช้ค่า default ของ paper ทุกตัว — ยกเว้น `SIGN_O` เพียงตัวเดียวที่ปรับสำหรับ domain ภาษามือไทย
...
| หัวข้อ | รายละเอียด | ผลที่คาดหวัง |
| --- | --- | --- |
| **Train segmentation model บนภาษามือไทย** | ปัจจุบันใช้โมเดลที่ train บน BSL → ต้องปรับ `SIGN_O` แบบ ad hoc | ไม่ต้องปรับ threshold เอง; ขอบเขต segment แม่นยำขึ้น |
| **ต่อ ASR pipeline** | ใช้ subtitle จาก speech recognition จริงแทน CC tier สำเร็จรูป | pipeline ครบวงจร ไม่ต้องมี annotation ล่วงหน้า |
| **ทดสอบ `sign_clip_embedding` mode** | ใช้ visual feature ของท่ามือจริงๆ ใน similarity matrix | +6% F1@0.50 (จาก BOBSL benchmark), ไม่ต้องใช้ gloss label |
| **เพิ่ม dataset ภาษามือไทย** | GT ปัจจุบันเป็น reference alignment เท่านั้น | วัดคุณภาพ alignment ได้ fair กว่า ด้วย independent human annotation |
| **Fine-tune ค่า hyperparameter** | ทดสอบ training mode ของ SEA บน Thai SL corpus | ได้ค่า optimal สำหรับภาษามือไทยโดยเฉพาะ |

---

## แนวทางการนำเสนอโครงการ

### โครงสร้างการนำเสนอที่แนะนำ

#### 1. ปัญหา (2–3 นาที)

- เปิดด้วยวิดีโอภาษามือสั้นๆ แล้วถามว่า "subtitle ตรงกับมือที่เคลื่อนไหวจริงหรือไม่?"
- อธิบายว่า subtitle ส่วนใหญ่ sync กับเสียงพูด ไม่ใช่ท่ามือ → ช้าหรือเร็วกว่า 2–5 วินาที

#### 2. Pipeline ทั้ง 4 ขั้น (5–7 นาที)

- แสดงแผนภาพ pipeline จากบนลงล่าง: Video → Pose → Sign Segments → Embeddings → DP Alignment
- เน้นว่าแต่ละขั้นผลิตไฟล์อะไร (`.pose`, `.eaf`, `.npy`, `.vtt`)

#### 3. หัวใจของ Method — DP Alignment (3–5 นาที)

- แสดงสูตร cost function `φ(tᵢ, sₗ:ᵣ)` พร้อมอธิบายแต่ละ term
- อธิบาย Numba JIT: "ช้า 10–45 วินาทีครั้งแรก เพราะ compile เป็น native code — ไม่ใช่ bug"

#### 4. ผลลัพธ์ (3–4 นาที)

- เปิด notebook แสดง **timeline chart** (Section 8) — เป็นภาพที่เห็นชัดที่สุด
- แสดงตัวเลข F1@0.50 = 89.41%, Mean |Δstart| = 0.45 วินาที

#### 5. สิ่งที่ปรับจาก Paper (2 นาที)

- เน้นว่าใช้ค่า default ของ paper ทุกตัว ยกเว้น `SIGN_O=70` (paper ใช้ 50)
- อธิบายเหตุผล: โมเดล train บน BSL → over-segments Thai SL → เพิ่ม threshold

### จุดสำคัญที่ต้องเน้น

| จุด | เหตุผลที่สำคัญ |
| --- | --- |
| **CC tier ≠ CC_Aligned** | ถ้าใส่ผิด tier → metrics ผิดทั้งหมด (119 cues vs GT 172 cues) |
| **SIGN_O=70 ไม่ใช่ error** | เป็นการ adapt method สำหรับ domain ใหม่ อธิบายได้ด้วย paper context |
| **89.41% ≠ 50.68% ของ Paper** | GT ต่างประเภทกัน — อย่านำไปเปรียบเทียบโดยตรง |
| **Numba warmup** | First run ช้าเป็นเรื่องปกติ — อธิบายก่อนรัน live demo |
| **text_embedding ใช้ SentenceTransformer 384-dim** | ไม่ใช่ SignCLIP; ถ้าต้องการ visual features → ใช้ `sign_clip_embedding` แทน |

### Demo Live ใน Notebook

เปิด [notebooks/example_alignment_usage.ipynb](notebooks/example_alignment_usage.ipynb) แล้วชี้ cell by cell:

- **Section 2 (Cell 4):** แสดง cue-count check — "172 cues ✓ ตรงกับ GT"
- **Section 4 (Cell 9):** แสดง threshold comparison table — อธิบาย SIGN_O=70
- **Section 6 (Cell 15):** แสดงสูตร DP — อธิบาย cost function ทุก term
- **Section 6 (Cell 17):** รัน DP — แสดงว่า Numba warmup ช้า 10–45s ครั้งแรก
- **Section 7 (Cell 19):** แสดงผล metrics — "✓ Cue counts match"
- **Section 8 (Cell 24):** แสดง **timeline chart** — จุดไฮไลท์ของการนำเสนอ
