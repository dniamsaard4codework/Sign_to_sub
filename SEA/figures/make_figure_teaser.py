...
def compute_y_positions_top_down(
    ...
    # Determine default top padding and inter-row spacings
    if big_space_mode:
        default_top_pad = large_spacing
        spacings = [large_spacing] * (num_tiers - 1)
    else:
        default_top_pad = small_spacing
        spacings = [small_spacing] * (num_tiers - 1)

    # Allow overriding the top padding independently of inter-row spacing
    top_pad = default_top_pad if top_pad_override is None else top_pad_override
...
def plot_timeline_with_tiers(
    ...
    frame_height,
    tier_data,
    include_sign_gt=False,
    include_subtitle_gt=True,
    teaser=False
):
    frames = []
    widths = []
    ...
    # ===== Tier configuration =====
    if teaser:
        # Teaser: compact 3 rows, with label renaming
        base_tiers = [
            ('CSLR',              'SIGN',               '#f6d88b'),  # use SIGN color
            ('SUBTITLE',          'SUBTITLE',           '#ccebd0'),
            ('SUBTITLE_SHIFTED',  'SUBTITLE\naligned', '#b8d8ff'),  # break label into two lines
        ]
        # Ignore GT include flags in teaser
        include_sign_gt_local = False
        include_subtitle_gt_local = False
    else:
        base_tiers = [
        ...
        include_sign_gt_local = include_sign_gt
        include_subtitle_gt_local = include_subtitle_gt

    # Apply include flags (only relevant when not teaser)
    tier_names, tier_labels, tier_colors = [], [], []
    for name, label, color in base_tiers:
        if not teaser:
            if name == 'CSLR' and not include_sign_gt_local:
                continue
            if name == 'SUBTITLE_CORRECTED' and not include_subtitle_gt_local:
                continue
        tier_names.append(name)
        tier_labels.append(label)
        tier_colors.append(color)

    # Teaser-specific hack: remove first-row (CSLR) and third-row (SUBTITLE_SHIFTED) segments starting at or after 79s
    if teaser:
        cutoff_time = 79.0
        for t in ('CSLR', 'SUBTITLE_SHIFTED'):
            if t in tier_data:
                tier_data[t] = [(s, e, txt) for (s, e, txt) in tier_data[t] if s < cutoff_time]

    # On-the-fly hack: align first 'amphibians' SIGN start to the first SUBTITLE_aligned start
    try:
        sub_shifted_segments = tier_data.get('SUBTITLE_SHIFTED', [])
        if sub_shifted_segments:
            first_sub_aligned_start = min(s for s, e, t in sub_shifted_segments)
            # Prefer adjusting the tier actually shown (CSLR in teaser, else SIGN/CSLR if present)
            for sign_tier in ['CSLR', 'SIGN']:
                if sign_tier in tier_names and tier_data.get(sign_tier):
                    segs = tier_data[sign_tier]
                    # Find the first segment whose text matches/contains 'amphibians'
                    idx = next((i for i, (s, e, txt) in enumerate(segs)
                                if isinstance(txt, str) and txt.strip().lower().startswith('amphibians')), None)
                    if idx is None:
                        idx = next((i for i, (s, e, txt) in enumerate(segs)
                                    if isinstance(txt, str) and 'amphibian' in txt.lower()), None)
                    if idx is not None:
                        s, e, txt = segs[idx]
                        # Set new start to the first aligned subtitle start, clamped to be < end
                        new_s = max(start_time, min(first_sub_aligned_start, e - 1e-3))
                        # Keep the segment valid
                        if new_s < e:
                            segs[idx] = (new_s, e, txt)
                        break
    except Exception:
        pass

    # Additional teaser-specific hack: clip the aligned sentence's end to the end of the sign "them"
    if teaser:
        try:
            cslr_segs = tier_data.get('CSLR', []) or []
            sub_shifted = tier_data.get('SUBTITLE_SHIFTED', []) or []
            # Find the LAST occurrence of CSLR segment labeled exactly "them" (by latest start time)
            them_end = None
            matches = [(s, e) for s, e, txt in cslr_segs if isinstance(txt, str) and txt.strip().lower() == 'them']
            if matches:
                latest_them = max(matches, key=lambda t: t[0])
                them_end = latest_them[1]
            # If found, clip the target aligned sentence to end at that time (no extension)
            if them_end is not None and sub_shifted:
                target_sentence = "Many people loathe or even hate them, but a few stalwart individuals love them."
                for i, (s, e, txt) in enumerate(sub_shifted):
                    if isinstance(txt, str) and txt.strip() == target_sentence:
                        new_e = min(e, them_end)
                        # Ensure the segment remains valid
                        if new_e <= s + 1e-3:
                            new_e = s + 0.1
                        sub_shifted[i] = (s, new_e, txt)
                        break
                tier_data['SUBTITLE_SHIFTED'] = sub_shifted
        except Exception:
            pass

    num_tiers = len(tier_names)
    tier_height = 38
    small_spacing = 40 if teaser else 6
    large_spacing = int(1.5 * tier_height)

    # Spacing policy
    if teaser:
        big_space_mode = False  # always compact for teaser
    else:
        big_space_mode = (not include_sign_gt_local) and (not include_subtitle_gt_local)
    ...
    top_pad_override = 6 if teaser else None
    y_positions, y_last_top, top_pad, spacings = compute_y_positions_top_down(
        ...
        small_spacing=small_spacing, large_spacing=large_spacing, top_pad_override=top_pad_override
    )
    ...
    # Timestamps go below the last row:
    timestamp_y = y_last_bottom - 14
    # Slightly smaller extra margin when teaser
    extra_margin_top = 55 if not teaser else 40
    extra_margin_bottom = 20 if not teaser else 16
    fig_height = frame_height + (tiers_block_height + extra_margin_top + extra_margin_bottom)
    ...
    for j, (tier, label, color) in enumerate(zip(tier_names, tier_labels, tier_colors)):
        ...
        for seg_start, seg_end, text in segments:
            ...
            # text
            if text:
                # Slightly reduce available width to keep text safely inside rounded corners
                available_width = max(1, width - 6)
                is_subtitle_tier = tier in ('SUBTITLE', 'SUBTITLE_SHIFTED', 'SUBTITLE_CORRECTED')
                px_per_char = 9 if is_subtitle_tier else 7
                wrapped_text = wrap_text(text, available_width, font_size=12, px_per_char=px_per_char, max_lines=2)
            else:
                wrapped_text = ''

            # Skip text for SIGN-like tiers if the segment is clipped by the visible window
            is_sign_tier = tier in ('SIGN', 'CSLR')
            is_clipped = (seg_start < start_time + 1e-3) or (seg_end > plot_end - 1e-3)
            if is_sign_tier and is_clipped:
                wrapped_text = ''
...
def main():
    ...
    parser.add_argument('--teaser', action='store_true', help='Use compact 3-row teaser (SIGN, SUBTITLE, SUBTITLE_aligned)')
    ...
    if args.teaser:
        ...
        mode_str = f"{signgt_str}_{subgt_str}"

    output_pdf = os.path.join(
        args.working_dir,
        f"{args.video_id}_{int(args.start)}_{int(args.end)}_{mode_str}.pdf"
    )
    ...
    # Collect only the tiers we might use
    all_possible_tiers = [
        'SIGN',
        'CSLR',
        'SUBTITLE',
    ...
    tier_data = load_elan_segments(input_eaf, args.start, args.end, tiers=all_possible_tiers)
